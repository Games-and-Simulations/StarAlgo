...
void CombatCommander::myUpdateSquads() {
    ...
    for (int i = 0; i < bestActions.size(); ++i) {
        for (int y = 0; y < groupIds.size(); ++y) {
            if (groupIds[y] == bestActions[i].pos) {
                //our set
                BWAPI::Unitset squadSet = searchAlg->idToSquad[bestActions[i].pos];
                auto& targetPosition = gs.regman->getCenterRegionId(bestActions[i].action.targetRegion);
    ...
    //    SquadOrder order(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[i].action.targetRegion), 800, "MC");
    //    for (auto& unit : _combatUnits) {
    //        BWAPI::Position pos = unit->getRegion()->getCenter();
    //        BWAPI::Position pos2 = gs.regman->_regionFromID[gs._army.friendly[bestActions[i].pos]->regionId]->getCenter();
    ...
    //    five = SquadOrder(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[i].action.targetRegion), 800, "MC");
    //    for (auto& unit : _combatUnits) {
    ...
    //SquadOrder oneOrder(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[groupId1].action.targetRegion), 800, "MC");
    //squads[0]->setSquadOrder(oneOrder);
    //SquadOrder twoOrder(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[groupId2].action.targetRegion), 800, "MC");
    //squads[1]->setSquadOrder(twoOrder);
    //SquadOrder threeOrder(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[groupId3].action.targetRegion), 800, "MC");
    //squads[2]->setSquadOrder(threeOrder);
    //SquadOrder fourOrder(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[groupId4].action.targetRegion), 800, "MC");
    //squads[3]->setSquadOrder(fourOrder);
    //SquadOrder fiveOrder(SquadOrderTypes::Attack, gs.regman->getCenterRegionId(bestActions[groupId5].action.targetRegion), 800, "MC");
...
*/
    if (squad != nullptr /*&& BWAPI::Broodwar->getFrameCount() % 400 == 0*/) {
        if (true) {
            ...
            if (bestActions.size())
                return gs.regman->getCenterRegionId(bestActions.front().action.targetRegion);
