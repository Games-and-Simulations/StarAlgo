...
void ProductionManager::manageBuildOrderQueue() 
{
	...
	{
		BWAPI::Unit producer;
		if (currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Zerg_Lair ||
			currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Zerg_Hive)
		{
			producer = getProducer(currentItem.metaType, BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation()));
		}
		else
		{
	...
	/*		for (BWTA::Chokepoint * choke : BWTA::getChokepoints())
			...
			*/
			for (auto& u : units)
			{
				...
				{
					BuildingManager::Instance().addBuildingTask(t.getUnitType(), u->getTilePosition(), item.isGasSteal);
					hasLairOrHive = true;
					break;
				}
			}
